    ...
    try {
        ...
        if (error instanceof AxiosError) {
            if (error.response) {
                ...
                console.error('【API状态检查】服务器响应错误:', error.response.status, error.response.data);
            }
            else {
                // 请求已发出，但没有收到响应（例如网络不通或超时）
                console.error('【API状态检查】网络或请求设置错误:', error.message);
            }
        }
        else {
            // 其他未知错误
            console.error('【API状态检查】捕获到未知错误:', JSON.stringify(error));
        }
        // 发生任何错误都表示连接失败
        return false;
    }
