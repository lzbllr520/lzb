...
function bigintToValue(bigint) {
  ...
  if (h[0] !== '-') {
    ...
    mask = new jsbn.BigInteger(mask, 16)

    // 对绝对值取反，加1
    h = mask.xor(bigint).add(jsbn.BigInteger.ONE)
...
function getL(str, start) {
  ...
  if (!l) return -1
  const bigint = +l[0] < 8 ? new jsbn.BigInteger(l, 16) : new jsbn.BigInteger(l.substr(2), 16)
...
function decodeDer(input) {
    ...
    const r = new jsbn.BigInteger(vR, 16)
    const s = new jsbn.BigInteger(vS, 16)
