...
class ECPointFp {
  constructor(curve, x, y, z) {
    ...
    this.z = z == null ? jsbn.BigInteger.ONE : z
    this.zinv = null
  ...
  isInfinity() {
    ...
    return this.z.equals(jsbn.BigInteger.ZERO) && !this.y.toBigInteger().equals(jsbn.BigInteger.ZERO)
  }
  ...
  add(b) {
    ...
    if (jsbn.BigInteger.ZERO.equals(w3)) {
      if (jsbn.BigInteger.ZERO.equals(w6)) {
...
class ECCurveFp {
  ...
  decodePointHex(s) {
    switch (parseInt(s.substr(0, 2), 16)) {
      ...
      case 3:
        // 压缩
        const x = this.fromBigInteger(new jsbn.BigInteger(s.substr(2), 16))
        ...
        let y = this.fromBigInteger(x.multiply(x.square()).add(
          ...
          .modPow(
            this.q.divide(new jsbn.BigInteger('4')).add(jsbn.BigInteger.ONE), this.q
          ))
        // 算出结果 2 进制最后 1 位不等于第 1 个字节减 2 则取反
        if (!y.toBigInteger().mod(TWO).equals(new jsbn.BigInteger(s.substr(0, 2), 16).subtract(TWO))) {
          y = y.negate()
        }
      ...
      case 7:
        ...
        return new ECPointFp(this, this.fromBigInteger(new jsbn.BigInteger(xHex, 16)), this.fromBigInteger(new jsbn.BigInteger(yHex, 16)))
      default:
        // 不支持
        return null
    }
