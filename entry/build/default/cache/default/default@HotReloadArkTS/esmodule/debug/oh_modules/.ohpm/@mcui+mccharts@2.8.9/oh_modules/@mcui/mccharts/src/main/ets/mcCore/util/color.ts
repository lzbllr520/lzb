...
function getOpacity(r) {
    const t = g(r);
    return isRgba(t) ? Number(t.toLowerCase().split(",").slice(-1)[0].replace(/[)|\s]/g, "")) : 1;
}
...
export { darken, fade, getColorFromRgbValue, getColorParseIntValue, getOpacity, getRgbValue, getRgbaValue, isHex, isRgb, isRgbOrRgba, isRgba, lighten, toHex, toRgb };
export default {
    darken,
    fade,
