...
import jsbn from '@normalized:N&&&@yyz116/jsbn/index&1.0.2';
import {Asn1} from '@normalized:N&&&@yyz116/sm-crypto/src/main/js/sm2/asn1&1.0.2';
import * as utils from '@normalized:N&&&@yyz116/sm-crypto/src/main/js/sm2/utils&1.0.2';
import { sm3 } from '@normalized:N&&&@yyz116/sm-crypto/src/main/js/sm2/sm3&1.0.2';
...
function doEncrypt(msg, publicKey, cipherMode = 1) {
  ...
  const k = new jsbn.BigInteger(keypair.privateKey, 16) // 随机数 k
...
function doDecrypt(encryptData, privateKey, cipherMode = 1, {
  ...
  privateKey = new jsbn.BigInteger(privateKey, 16)
...
function doSignature(msg, privateKey, {
  ...
  const dA = new jsbn.BigInteger(privateKey, 16)
  const e = new jsbn.BigInteger(hashHex, 16)
  ...
  do {
    ...
    } while (r.equals(jsbn.BigInteger.ZERO) || r.add(k).equals(n))

    // s = ((1 + dA)^-1 * (k - r * dA)) mod n
    s = dA.add(jsbn.BigInteger.ONE).modInverse(n).multiply(k.subtract(r.multiply(dA))).mod(n)
  } while (s.equals(jsbn.BigInteger.ZERO))
...
function doVerifySignature(msg, signHex, publicKey, {der, hash, userId} = {}) {
  ...
  if (der) {
    ...
    r = new jsbn.BigInteger(signHex.substring(0, 64), 16)
    s = new jsbn.BigInteger(signHex.substring(64), 16)
  }
  ...
  if (t.equals(jsbn.BigInteger.ZERO)) return false
...
function getPublicKeyFromPrivateKey(privateKey) {
  const PA = G.multiply(new jsbn.BigInteger(privateKey, 16))
...
function getPoint() {
  ...
  keypair.k = new jsbn.BigInteger(keypair.privateKey, 16)
