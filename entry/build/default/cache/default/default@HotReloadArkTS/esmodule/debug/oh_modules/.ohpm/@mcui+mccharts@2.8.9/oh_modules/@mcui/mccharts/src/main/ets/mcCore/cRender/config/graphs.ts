import beziercurve from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/bezierCurve/index&2.8.9";
import { deepClone, eliminateBlur, checkPointIsInCircle, getTwoPointDistance, checkPointIsInSector, getRegularPolygonPoints, checkPointIsInPolygon, checkPointIsNearPolyline, checkPointIsInRect, roundRectFn } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/cRender/plugin/util&2.8.9";
import { drawPolylinePath, drawBezierCurvePath } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/cRender/plugin/canvas&2.8.9";
...
export const circle = {
    ...
    hoverCheck(position, obj) {
        const { shape } = obj;
        const { rx, ry, r } = shape;
        return checkPointIsInCircle(position, rx, ry, r);
    },
    setGraphCenter(e, obj) {
        const { shape, style } = obj;
        const { rx, ry } = shape;
...
export const ellipse = {
    ...
    hoverCheck(position, obj) {
        ...
        const leftFocusPoint: number[] = [rx - c, ry];
        const rightFocusPoint: number[] = [rx + c, ry];
...
export const roundRect = {
    ...
    draw({ ctx }, { shape }) {
        ctx.beginPath();
        let { x, y, w, h, r = 2 } = shape;
        roundRectFn(ctx, x, y, w, h, r || 0);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
    },
...
export const regPolygon = {
    ...
    draw({ ctx }, { shape, cache }) {
        ...
        if (!cache.points || cache.rx !== rx || cache.ry !== ry || cache.r !== r || cache.side !== side) {
            const points = getRegularPolygonPoints(rx, ry, r, side, Math.PI * -0.5, shape.startAngle || 0);
...
export const smoothline = {
    ...
    draw({ ctx }, { shape, cache }) {
        ...
        ctx.beginPath();
        if (bezierCurve) {
            drawBezierCurvePath(ctx, bezierCurve.slice(1), bezierCurve[0], false, close);
        }
        else {
            drawPolylinePath(ctx, points);
        }
...
export const text = {
    ...
    draw({ ctx }, { shape }) {
        ...
        const fontSize = font ? parseInt(font.replace(/\D/g, '')) : 12;
        let [x, y] = position;
...
export const textEnd = {
    ...
    draw({ ctx }, { shape }) {
        let { content, points, maxWidth, rowGap } = shape;
        // 示例：使用函数计算特定帧对应的索引
        // const dataCount = 18;
        // const frameCountTotal = 30; // 总帧数
        //
        // const currentFrame = index;
        // const flattenedArray = manuallyFlattenTo2D(points);
        // const i = getIndexForFrame(dataCount, frameCountTotal, currentFrame);
        // if (index < 30) {
        //   ctx.fillText('测试', ...flattenedArray[i])
        //   index += 1
        // } else {
        //   ctx.fillText('测试', ...flattenedArray[i])
        // }
