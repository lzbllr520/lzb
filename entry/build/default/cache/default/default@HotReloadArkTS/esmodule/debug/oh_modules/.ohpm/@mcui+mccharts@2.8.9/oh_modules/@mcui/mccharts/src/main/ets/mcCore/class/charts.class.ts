import "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/extend/index&2.8.9";
import CRender from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/cRender/index&2.8.9";
import { deepClone } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/util/common&2.8.9";
import { findNearestPointIndex, isPointInSector } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/util/index&2.8.9";
import { mergeColor, title, grid, axis, radarAxis } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/core/index&2.8.9";
import { pie, line, bar, radar, gauge, legend, tooltip, dataZoom } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/core/index&2.8.9";
import type { OptionInterface, SeriesInterface, InterfaceObj } from '../../utils/chartInterface';
export default class Charts {
    ...
    ctxClick(event, tipCallback) {
        ...
        try {
            ...
            if (x < gridX - 10 || y < gridY)
                return;
            if (x > gridX + w + 10 || y > gridY + h)
                return;
        }
        catch (e) {
    ...
    ctxClickLegend(event, types) {
        const { x, y } = event;
        const position = [x, y];
        const hoverAbleGraphs = this.render.graphs.filter(graph => (graph.hover && (typeof graph.hoverCheck === 'function' || graph.hoverRect)));
    ...
    tipLineAndBar(event, types, axisData, x, y, linePoint, tooltip, tipCallback) {
        ...
        if (activeIndex !== null) {
            ...
            // 添加提示线条，或者背景
            const { axisPointer: { type } } = tooltip.series;
    ...
    bindZoom(offset, typeFlag) {
        ...
        if (!axisData || !axisData.length || !this.dataZoom)
            return;
    ...
    bindZoomScale(scale) {
        ...
        if ((xEnd - xStart <= 1 && delta > 0) || (xStart === 0 && xEnd === xl && delta < 0))
            return;
        ...
        if (Math.abs(this.zoomAccumulator) >= 1) {
            ...
            if (newXStart === xStart && newXEnd === xEnd)
                return;
