import curves from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/transition/config/curves&2.8.9";
const defaultTransitionBC = 'linear';
/**
 * @description 根据动画的开始状态、结束状态和缓动曲线获取 N 帧动画状态
 * @param {string | Array} tBC             缓动曲线名称或数据
 * @param {number | Array | Object} startState 动画开始状态
 * @param {number | Array | Object} endState   动画结束状态
 * @param {number} frameNum                动画帧数
 * @param {boolean} deep                   是否使用递归模式
 * @return {Array | boolean} 每帧动画的状态（无效输入将返回 false）
 */
 ...
 * @param {String} tBC      转换贝塞尔曲线的名称
 * @param {Any} startState  转换开始状态
 * @param {Any} endState    转换结束状态
 * @param {Number} frameNum 转换帧的数量
...
/**
 * @description 获取转换贝塞尔曲线数据
 * @param {string} tBC 转换贝塞尔曲线的名称
 * @return {Array<[number, number]>} 贝塞尔曲线数据
 */
...
/**
 * @description 获取每一帧的状态进度
 * @param {Array<[number, number]>} bezierCurve 转换贝塞尔曲线数据
 * @param {number} frameNum 转换帧数
 * @return {Array<number>} 每一帧的状态进度
 */
