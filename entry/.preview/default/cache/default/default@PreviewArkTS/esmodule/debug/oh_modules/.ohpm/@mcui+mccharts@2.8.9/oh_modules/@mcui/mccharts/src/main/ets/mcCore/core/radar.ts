...
import { radarConfig } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/config/index&2.8.9";
import { getCircleRadianPoint } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/cRender/plugin/util&2.8.9";
import { getColorFromRgbValue, getRgbaValue } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/util/color&2.8.9";
import { deepMerge, initNeedSeries } from "@normalized:N&&&@mcui/mccharts/src/main/ets/mcCore/util/index&2.8.9";
...
function getLabelShape(radarItem, i) {
    ...
    if (formatterType === 'string')
        labelText = getFormatterVal(callbackSendable, formatter, { name: labelText }, i);
    if (formatterType === 'function')
        labelText = formatter({ name: labelText }, i);
    return {
        content: labelText,
        position
    };
