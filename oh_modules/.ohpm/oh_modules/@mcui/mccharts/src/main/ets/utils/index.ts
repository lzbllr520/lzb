...
export const deepCopy = function (data) {
  ...
  if (t === 'array') {
    ...
    for (const i in data) {
      if (i === 'data') {
        o[i] = JSON.parse(JSON.stringify(data[i]))
      } else {
        o[i] = deepCopy(data[i])
      }
    }
...
export const getOptionCallback = (data, CallbackList) => {
  for (const key in data) {
    ...
    if (itemType === 'array') {
      for (let i = 0; i < item.length; i++) {
        item[i] = getOptionCallback(item[i], CallbackList)
      }
    } else if (itemType === 'object') {
      data[key] = getOptionCallback(item, CallbackList)
    } else if (itemType === 'string' && item.indexOf('function:') !== -1) {
      data[key] = CallbackList[item]
    }
  }
  return data
}
